...
teleportOverlay.ImageTransparency = 0 
teleportOverlay.ScaleType = Enum.ScaleType.Stretch 
...
local Window = Luna:CreateWindow({Title = "Steal a Brainrot | H4x Hub", HidePremium = false, SaveConfig = true, ConfigFolder = "OrionTest"})
...
local Section = Tab:CreateSection("Stealer Section")




Tab:CreateToggle({
	Title = "Float (Need 7 rebirths)",
	CurrentValue = false,
...
Tab:CreateToggle({
	Title = "Aimbot (Web Slinger, LaserCape)",
	CurrentValue = false,
	Callback = function(state)
		local player = game:GetService("Players").LocalPlayer
        local Players = game:GetService("Players")
...
if myPlot then
    ...
    for _, obj in ipairs(purchases:GetDescendants()) do
        if obj:IsA("TextLabel") and obj.Title = = "RemainingTime" then
            countdownLabel = obj
            break
        end
    end
...
local function walkToMyPlot()
    ...
    local targetPos = myHitbox.Position + Vector3.new(0, 2, 0) 
...
local Section = Tab:CreateSection("MyBase Section")
...
local function getRemainingSeconds(plot)
    for _, obj in ipairs(plot:GetDescendants()) do
        if obj:IsA("TextLabel") and obj.Title = = "RemainingTime" and obj.Visible then
...
local Section = Tab2:CreateSection("Steal Helper")
...
local TimerParagraph = Tab2:CreateParagraph("Lock Time: ", "Timer Paragraph")
...
local selectedPets = {}
local SelectBrainrot = Tab2:CreateDropdown({Title = "Select Brainrot",
    CurrentOption = {},
    Values = allPets,
...
Tab2:CreateToggle({Title = "StealToggle", CurrentValue = false, Callback=function(v) _G.AutoStealEnabled = v end})
Tab2:CreateToggle({Title = "StealAllToggle", CurrentValue = false, Callback=function(v) _G.AutoStealAllEnabled = v end})
Tab2:CreateToggle({Title = "KickToggle", CurrentValue = false, Callback=function(v) kickEnabled=v promptTriggered=false end})
...
local Section = Tab3:CreateSection("Purchase Moving Animals")
...
local SelectBrainrot = Tab3:CreateDropdown({Title = "Select Brainrot",
    CurrentOption = {},
    Values = allPets,
    MultipleOptions = true,
    Callback = function(values)
...
Tab3:CreateDropdown({Title = "Select Collect Method",
    CurrentOption = "Tween",
    Values = {"Tween", "Walk"},
    Callback = function(v)
        selectedFarmMode = v
...
local Section = Tab4:CreateSection("ESP Toggles")
...
local Section = Tab3:CreateSection("Buy Items/Tools")
...
local Section = Tab5:CreateSection("Webhook Management")
...
Tab5:CreateInput({
    Title = "Webhook URL",
    CurrentValue = Config.Webhook ~= "" and Config.Webhook or "Enter webhook here...",
    TextDisappear = false,
    Callback = function(value)
        if value ~= "" then
...
local SelectBrainrot = Tab5:CreateDropdown({Title = "Select Brainrot",
    Values = allPets,
    MultipleOptions = true,
    CurrentOption = {},
    Callback = function(values)
...
Tab5:CreateDropdown({
    Title = "Select Method to log",
    Values = {"Spawned Brainrots", "Current Server"},
    CurrentValue = "Spawned Brainrots",
...
Tab5:CreateToggle({
    Title = "Send embed to your server",
    CurrentValue = false,
    Callback = function(state)
...
Tab5:CreateToggle({
    Title = "Send embed with @everyone",
    CurrentValue = false,
    Callback = function(val)
...
local Section = Tab6:CreateSection("Current Server Section")



local TimerParagraph = Tab6:CreateParagraph("Server Time:", "Shows how long the server has been running")
local st = os.time()
...
local Section = Tab6:CreateSection("Current Server Actions")



local JobIDInput = Tab6:CreateInput({
    Title = "Job ID",
    CurrentValue = "",
    TextDisappear = true,
    Callback = function(Value)
...
local Section = Tab7:CreateSection("Brainrot Founder")
...
local function loadPets()
    ...
    if success and result then
        ...
        if #petData == 0 then
            table.insert(currentButtons, Tab7:CreateParagraph("No Pets Found", "NoPetsFound"))
            return
        end
        ...
        table.insert(currentButtons, Tab7:CreateParagraph("Error to Load","Error"))
    end
end
...
-- Initialize Luna autoloading/config
