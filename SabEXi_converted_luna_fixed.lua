...
local TimerParagraph = Tab:CreateParagraph("Lock Time: ", "Timer Paragraph")
...
local selectedPets = {}
local SelectBrainrot = Tab:CreateDropdown({
...
Tab:CreateToggle({Name="StealToggle", CurrentValue =false, Callback=function(v) _G.AutoStealEnabled = v end})
Tab:CreateToggle({Name="StealAllToggle", CurrentValue =false, Callback=function(v) _G.AutoStealAllEnabled = v end})
Tab:CreateToggle({Name="KickToggle", CurrentValue =false, Callback=function(v) kickEnabled=v promptTriggered=false end})
...
local TimerParagraph = Tab:CreateParagraph("Server Time:", "Shows how long the server has been running")
local st = os.time()
...
local function loadPets()
    ...
    if success and result then
        ...
        if #petData == 0 then
            table.insert(currentButtons, Tab:CreateParagraph("No Pets Found", "NoPetsFound"))
            return
        end
        ...
        table.insert(currentButtons, Tab:CreateParagraph("Error to Load","Error"))
    end
end
