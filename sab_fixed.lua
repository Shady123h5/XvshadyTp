...
local function safeRequire(module)
    ...
    if not ok then warn("Require failed:", module) return {} end
    return mod
end
...
teleportOverlay.ScaleType = Enum.ScaleType.Stretch -- skaliert das Bild Ã¼ber den ganzen Bildschirm
...
teleportLabel.Text = "H4X is Loading"
...
-- Ã°Å¸â€œÅ’ Services & Module
local plrs = game:GetService("Players")
local rs = game:GetService("ReplicatedStorage")
local plr = plrs.LocalPlayer

local plotController = safeRequire(rs.Controllers.PlotController)
local Animals = safeRequire(rs.Datas.Animals)
local sharedAnimals = safeRequire(rs.Shared.Animals)

-- Ã°Å¸â€œÅ’ Hilfsfunktionen
...
-- =========================================================
-- Ã°Å¸Å¸Â¢ Highlight Best Plot Animals
-- =========================================================
...
local OrionLib; do
    local ok, lib = pcall(function()
        return loadstring((function() local ok, result = pcall(function() return game:HttpGet("loadstring(game:HttpGet("https://raw.githubusercontent.com/jensonhirst/Orion/main/source"))()") end);if ok then return result else warn("HttpGet failed") return "" end end)())()
    end)
    OrionLib = ok and lib or { MakeWindow = function() warn("OrionLib failed, using dummy") return { Tab=function() return {} end } end }
end

local Window = OrionLib:MakeWindow({Name = "Steal a Brainrot | Denji Hub", HidePremium = false, SaveConfig = true, ConfigFolder = "OrionTest"})
...
if not clone.PrimaryPart then  
    ...
    if not clone.PrimaryPart then  
        warn("Kein PrimaryPart fÃ¼r Klon gefunden!")  
        return  
    end  
end  
...
local rechargeTime = 0.50    -- Sekunden bis sie sich wieder fÃ¼llt
local running = false
local percent = 1
...
local color
if percent > 0.6 then
color = Color3.fromRGB(0,255,0) -- GrÃ¼n
...
-- Progress-Bar einfÃ¤rben  
progressBar.BackgroundColor3 = color  

-- Part einfÃ¤rben (falls existiert)  
...
local function walkToMyPlot()
    ...
    local targetPos = myHitbox.Position + Vector3.new(0, 2, 0) -- leicht drÃƒÂ¼ber
    ...
    while true do
        ...
        if toggleActive and not debounce and countdownLabel and myHitbox then
            if resetableCountdowns[countdownLabel.Text] then
                ...
                -- Respawn fÃƒÂ¼r sichere Position
                local char = player.Character or player.CharacterAdded:Wait()
                ...
                task.spawn(function()
                    ...
                    task.wait(6) -- kurze Pause, bevor wieder mÃƒÂ¶glich
                    debounce = false
                end)
            end
        end
...
local RarityPets = {
    Common = {"Noobini Pizzanini", "Fluriflura", "LirilÃ¬ LarilÃ ", "Pipi Kiwi", "Pipi Corny", "Svinina Bombardino", "Talpa Di Fero", "Tim Cheese"},
...
local function sendWebhook(name, price, rarity, mutation, mps)
    ...
    if Config.SendEveryone then
        content = "@everyone âœ… Successfully Sent SAB Notify"
...
-- Dropdown fÃƒÂ¼r Brainrot (alle Pets)
...
-- Discord Copy Button
Tab6:AddButton({
    Name = "Denji Hub Discord Server",
    Callback = function()
        setclipboard("https://discord.gg/M4hz7PNUzn")
...
-- Ã°Å¸â€â€” API Endpoint - Dein Replit URL
...
------------------------------------------------------------
-- Pets laden + Buttons einfÃƒÂ¼gen (Uma Button Style)
------------------------------------------------------------
...
local function loadPets()
    local success, result = pcall(function()
        return (function() local ok, result = pcall(function() return game:HttpGet(getgenv() end);if ok then return result else warn("HttpGet failed") return "" end end)().websiteEndpoint:gsub("/api", "/Brainrot/pets_found.txt"))
    end)
    ...
    if success and result then
        ...
        for i, entry in ipairs(petData) do
            ...
            local title = (mutation ~= "" and "(" .. mutation .. ") " or "") .. petName .. " | Ã°Å¸â€™Â° " .. value
            local info = "Ã°Å¸â€˜Â¥ " .. (entry.players or "0/0") .. " | Ã¢ÂÂ° " .. (entry.timestamp or "??")
